...
public class LogAnnotation {
    ...
    @Before("execution(* *.*(..)) && @annotation(com.example.demo.LogMethod)")
    public void logMethodCall(JoinPoint joinPoint) {
        System.out.println("Called " + joinPoint);
    }
    ...
    private void logEvent(JoinPoint joinPoint, Method method) {
        ...
        if (when != null) {
            System.out.println("WHEN " + replaceArgs(when.value(), methodArgs));
        }
        Then then = method.getAnnotation(Then.class);
        if (then != null) {
            System.out.println("THEN " + replaceArgs(then.value(), methodArgs));
        }
    }
